...
postDetails.appendChild(attendance)
    ...
    ////Working on the ATTEND BUTTON EVENT LISTENER/////
...
commentCountSpan.textContent = `${post.commentsCount} comments`;
...
btns.appendChild(commentBtnContainer);
btns.appendChild(likeBtnContainer);

    postDetails.appendChild(btns);
    container.appendChild(postDetails)///////// APPENDING
    ////adding event listener to comment button
    commentBtnSVG.addEventListener('click', (e) =>{
        e.preventDefault();
        commentsSection.classList.remove("hide-comment-section");
        document.body.style.overflow='hidden';
    })
